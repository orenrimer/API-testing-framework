...
class TestGetCustomerSmoke:
    ...
    def test_get_existing_customer(self, setup):
        ...
        try:
            ...
            assert response_json["id"] == customer_id, logging.error(
                f"Invalid response. expected customer 'id': {customer_id} "
    ...
    def test_get_non_existing_customer(self, setup):
        ...
        try:
            ...
            customer_db = self.customer_db.select_random_customer(order_by="ID")

            if not customer_db:
                raise Exception("No customers found in database.")
            # incrementing the last customer ID result in a non-existing customer ID
            customer_id = customer_db[0]["ID"] + 1
            response_json = self.request_handler.get_by_id(
                self.endpoint, customer_id, expected_status_code=404
            )
            assert (
                "code" in response_json
                and response_json["code"] == "woocommerce_rest_invalid_id"
            ), logging.error(
                f"Invalid response, expected response code 'woocommerce_rest_invalid_id'"
            )

            logging.info("SUCCESS::get non-existing customer")
        except Exception as e:
            logging.error(e)
            pytest.fail()
    ...
    def test_get_all_customers(self, setup):
        ...
        try:
            ...
            for idx, customer in enumerate(customers_id):
                assert (
                    idx < len(customers_db_ids) and customer == customers_db_ids[idx]
                ), logging.error(
                    f"Invalid data, customer: 'id'={customer} not in database"
                )
