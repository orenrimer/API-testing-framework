...
class TestCreateOrdersSmoke:
    ...
    def test_create_order_with_new_user(self, setup):
        ...
        try:
            ...
            customer_json = self.request_handler.create(
                endpoint="customers", payload={"email": generate_random_email()}
            )
            assert customer_json
            order_json = self.request_handler.create(
                endpoint="orders",
    ...
    def verify_order_created(self, order_id, expected_products):
        ...
        if not self.order_db.select_order_by_order_id(order_id):
            raise Exception(f"order: 'id'={order_id} not in database")

        # check if the order contains all the expected items
        ...
        for itemId in tmp:
            if tmp.get(itemId, None) != db_order_products[itemId]:
                raise Exception(f"Item: 'id'={itemId} not found in database.")
