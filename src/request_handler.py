...
class RequestHandler:
    ...
    def create(self, endpoint, payload=None, expected_status_code=201):
        ...
        self.validate_status_code(response.status_code, expected_status_code, response_json)
    ...
    def delete(self, endpoint, id, expected_status_code=200):
        response = self.requests.delete(endpoint=f'{endpoint}/{id}', payload={"force": "true"})
        response_json = response.json()
        self.validate_status_code(response.status_code, expected_status_code, response_json)
        return response_json
