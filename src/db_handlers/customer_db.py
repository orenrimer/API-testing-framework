...
class CustomerDB:
    def __init__(self):
        self.db = DbUtils()
        self.TABLE_NAME = f"{DB_CONFIG['localhost']['table_prefix']}users"

    def select_customer_by_email(self, email):
        """
            select a customer from the db using email
        """
        sql = f"SELECT * FROM {DB_CONFIG['localhost']['database']}.{self.TABLE_NAME} WHERE user_email = '{email}';"
    ...
    def select_customer_by_id(self, customer_id):
        ...
        """
        sql = f"SELECT * FROM {DB_CONFIG['localhost']['database']}.{self.TABLE_NAME} WHERE ID = '{customer_id}';"
        response = self.db.select(sql)
        return response
    ...
    def select_random_customer(self, qty=1, params=None):
        ...
        """
        sql = f"SELECT * FROM {DB_CONFIG['localhost']['database']}.{self.TABLE_NAME} " \
    ...
    def select_all_customer(self):
        ...
        """
        sql = f"SELECT * FROM {DB_CONFIG['localhost']['database']}.{self.TABLE_NAME} WHERE ID <> 1;"
